...
El \textit{Optical Gravitational Lensing Experiment} (OGLE) es uno de los relevamientos fotométricos más extensos y longevos de la astronomía moderna. Iniciado en 1992 con el objetivo original de detectar eventos de microlente gravitacional, el proyecto evolucionó hasta convertirse en un relevamiento de gran escala orientado a identificar y estudiar toda forma de variabilidad estelar \citep{ogle2024}. Las observaciones se realizan con el telescopio de Varsovia de 1.3\,m, ubicado en el Observatorio de Las Campanas, en Chile, y se concentran en las regiones más densas del cielo austral: el bulbo galáctico, el disco galáctico y las Nubes de Magallanes.

A lo largo de su historia, el proyecto se ha dividido en cuatro fases con capacidades observacionales progresivamente mayores. La cámara actual, instalada en 2010 para iniciar OGLE-IV, cuenta con un mosaico de 32 chips CCD que cubre aproximadamente 3\,600 deg$^{2}$. Cada nueva fase representó un salto de un orden de magnitud respecto a la anterior: actualmente OGLE monitorea en torno a dos mil millones de fuentes puntuales y su base de datos acumula cerca de $1.2 \times 10^{12}$ mediciones fotométricas individuales, constituyéndola en una de las más grandes de la historia de la astronomía \citep{ogle2024}.

Una consecuencia directa de este monitoreo continuo es la \textit{OGLE Collection of Variable Stars} (OCVS), un catálogo creciente que contiene actualmente más de 1.1 millones de estrellas variables clasificadas en la Vía Láctea y las Nubes de Magallanes. El catálogo incluye, entre otros tipos, Cefeidas clásicas, estrellas RR Lyr\ae{}, variables $\delta$ Scuti, variables de largo período, binarias eclipsantes y novae enanas \citep{ogle2024}. Una característica que distingue a la OCVS de catálogos obtenidos mediante clasificación automática es que cada objeto ha sido revisado visualmente por astrónomos expertos, lo que le confiere un nivel de pureza y completitud excepcionalmente alto. Por este motivo, la colección es ampliamente utilizada como conjunto de entrenamiento y referencia para algoritmos de clasificación automática de estrellas variables basados en aprendizaje automático \citep{ogle2024}.

Para los fines del presente trabajo, los datos de la OCVS constituyen el conjunto de prueba principal. La riqueza de tipos variables y la calidad de las curvas de luz —con series temporales en bandas $I$ y $V$ que abarcan décadas de observación— proporcionan un banco de pruebas ideal para evaluar el desempeño del método de mínima entropía y compararlo con otros algoritmos de búsqueda de períodos.
