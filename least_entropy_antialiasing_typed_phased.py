...
def find_best_period(
    ...
    t_parts=4, u_parts=4,
    plot_entropies=False, eps=0, plot_periods=False, name=None, p_teo=0
):
    ...
    if plot_entropies:
        ...
        title = f"{name} — P = {best_period:.5f} d" if name else f"P = {best_period:.5f} d"

        ax1.plot(testing_periods, entropies)
        ax1.set_title(f"Periodograma — {title}")
        if p_teo != 0:
            ax1.axvline(p_teo, c="r", linestyle="--", label="Período real")
        ax1.axvline(best_period, c="black", linestyle="--", label="Período encontrado")
        ...
        ax2.axvline(1/best_period, c="black", linestyle="--", label="Frecuencia encontrada")
